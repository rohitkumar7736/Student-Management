...
def submit():
    if request.method == 'POST':
     ...
     if(btn=="Delete"):
      cur = mysql.connection.cursor()
      cur.execute("Delete from exam_result where course_id="+t3+"")
      mysql.connection.commit()
      cur.close()
      return redirect(url_for('index')) 
     if(btn=="Update"):
      cur = mysql.connection.cursor()
      cur.execute("Update exam_result set exam_id="+t1+", course_id="+t3+", marks='"+t4+"' where student_id="+t2+"")
      mysql.connection.commit()
      cur.close()
      return redirect(url_for('index')) 
     ...
     if(col=="All"):
      ...
      cur.execute("select * from exam_result where "+col+"='"+src+"'")
      data=cur.fetchall()
      cur.close()
      return render_template('exam_resultsearch.html',data=data)
