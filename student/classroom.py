...
def submit():
 ...
 if(btn=="Delete"):
  cur = mysql.connection.cursor()
  cur.execute("Delete from classroom where year='"+t2+"'")
  mysql.connection.commit()
  cur.close()
  return redirect(url_for('index')) 
 if(btn=="Update"):
  cur = mysql.connection.cursor()
  cur.execute("Update classroom set classroom_id="+t1+", grade_id="+t3+",section='"+t4+"',status='"+t5+"',remarks='"+t6+"',teacher_id='"+t7+"' where year='"+t2+"'")
  mysql.connection.commit()
  cur.close()
  return redirect(url_for('index')) 
 if(btn.lower()=="allsearch"):
  ...
  data=cur.fetchall()
  cur.close()
  return render_template('classroomsearch.html',data=data)     
 ...
 if(col=="All"):
  ...
  cur.execute("select * from Classroom where "+col+"='"+src+"'")
  data=cur.fetchall()
  cur.close()
  return render_template('classroomsearch.html',data=data)  
